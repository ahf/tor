...
static void
connection_printf_to_buf(stats_reporter_connection_t *conn, const char *format, ...)
{
  va_list ap;
  char *buf = NULL;
  int len;
 ...
 * stats_reporter_protocol_t instance. Returns STATS_REPORTER_PROTOCOL_UNKNOWN if the
 * input is an unknown stats reporter protocol.
 */
...
stats_reporter_graphite_tags(void)
{
  ...
  /* Add version tag. */
  tor_snprintf(buf + written, sizeof(buf) - written,  ";tor_version=%s", VERSION);
  written = strlen(buf);

  /* Add Git commit tag, if it's available. */
  if (strlen(tor_git_revision)) {
    tor_snprintf(buf + written, sizeof(buf) - written, ";tor_git_revision=%s", tor_git_revision);
    written = strlen(buf);
  }

  /* Add nickname, if it's available. */
  if (options->Nickname) {
    tor_snprintf(buf + written, sizeof(buf) - written, ";nickname=%s", options->Nickname);
    written = strlen(buf);
  }
