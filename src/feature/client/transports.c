...
launch_managed_proxy(managed_proxy_t *mp)
{
  ...
  process_set_name(mp->process, mp->argv[0]);
                      ...
                      int also_terminate_process)
{
  ...
  /* do we want to terminate our process? */
  if (also_terminate_process) {
    process_set_exit_callback(mp->process, NULL);
    process_terminate(mp->process);
  }
...
managed_proxy_exit_callback(process_t *process, process_exit_code_t exit_code)
{
  ...
  log_warn(LD_PT, "Pluggable Transport process terminated with status code %" PRIu64, exit_code);
