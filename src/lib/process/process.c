 ...
 * methods to configure it and run the process using <b>process_exec()</b>. */
process_t *
process_new(void)
{
  process_t *process;
...
process_exec(process_t *process)
{
  ...
  log_info(LD_PROCESS, "Starting new process: %s", process->process_name);
...
/** Set the name of a given process.  This function sets the
 * <b>process_name</b> of the given <b>process</b>. The process name is used to
 * be able to identify the given binary and is additionally used as
 * <b>argv[0]</b> in the argument list when the process is started. */
void
...
/** Get the current process name of the given process. */
const char *
...
/** Returns a list of arguments (excluding the process_name itself) from the
 * given process. */
const smartlist_t *
...
process_get_argv(const process_t *process)
{
  ...
  char *filename = process->process_name;
